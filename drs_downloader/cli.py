...
async def get_md5_and_size(session, url):
    async with session.get(url) as resp:
        md5_and_size = await resp.json(content_type=None)
...
async def get_md5s_and_sizes(file):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for result in results:
                    tasks.append(asyncio.ensure_future(get_md5_and_size(session, result)))
...
def get_signed_url(drs_client,Drs_ids):
    access_string = '/access/s3'
    whats_this = map(lambda x: x + access_string , Drs_ids)
    results = [list_obj for list_obj in whats_this]

    test_data = {'Gen3AcedDevelopment' : {'drs_client': drs_client,'drs_ids': results}}

    for (testname, test) in test_data.items():
        drsClient = test['drs_client']
        urls = []

        results = drsClient.get_access_url(test['drs_ids'])
        #print(results)
        exit()

        #new_variable =  map(lambda x: drsClient.get_access_url(x), test['drs_ids'])
        #results = [list_obj for list_obj in new_variable]
...
if __name__ == '__main__':
    ...
    start = time.time()
    download('~/Desktop/credentials.json','smol.txt','')
    end = time.time()
